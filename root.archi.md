...
- Toolchain: the root manifest declares `packageManager: "yarn@4.17.1"` and `engines.node: "22.x"`; `.yarnrc.yml` declares `nodeLinker: node-modules` — Plug'n'Play is deliberately not used, because `tsc`, `next build` and vitest all read the on-disk layout — plus `enableGlobalCache` (no zero-install, the cache stays out of git), `enableScripts` (sharp, esbuild, @parcel/watcher and protobufjs build native binaries at install time) and `yarnPath`. `.nvmrc` carries the same Node major for humans. Install is `yarn install`, and the yarn that runs it is the binary committed at `.yarn/releases/yarn-4.17.1.cjs`, which `yarnPath` names: any yarn on the PATH delegates to it, so nothing here assumes a globally installed yarn and nothing needs corepack — abandoned on 2026-08-01 because on Vercel it caches yarn inside the repository, where the root `"type": "module"` kills the install before a dependency resolves (`apps/robusta-build/README.md`). `packageManager` remains the declared version, read by a corepack shim or an editor; `yarnPath` is what decides the binary.
