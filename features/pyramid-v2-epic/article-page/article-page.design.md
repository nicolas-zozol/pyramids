...
- R-ARTICLEPAGE-62: Every colour, size, spacing and radius the page renders resolves to a custom property of `@robusta/pyramids-design-system`, and the page declares no literal of its own. Realizes BR-PYRAMID-6.
...
- AC-ARTICLEPAGE-07: Given a rendered article page, when Ada inspects its computed styles, then every colour, font size and spacing traces to a custom property of the design system, and the page's own CSS declares no literal. Realizes BR-PYRAMID-6 and R-ARTICLEPAGE-62.
...
- Gap 1: The consequence for whoever writes an article — HTML in a body renders as its text alone, silently — has no documented home. The story's Documentation updates carries a bullet for the Articles section of `apps/robusta-build/README.md`, but its stated why is the required `author` field alone, and docman executes that plan by its why.
- Proposition: widen that bullet's why to cover the raw-HTML rule too, so the Articles section tells a publisher that HTML written in a body loses its markup and that emphasis is written in markdown. This design states the mechanism for whoever implements it; the README states the rule for whoever writes the file. Both, not either.
- Rationale: the two readers need different things, and the location already exists in the plan — this widens a why rather than adding a place. A design never writes the story.
- Resolution:
