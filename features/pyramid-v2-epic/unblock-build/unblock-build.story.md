...
- 2026-08-02 — that documentation is delivered, commit `e4deb9a`: the four entries below are at the accomplished tense, and no document still tells a reader to run `corepack enable`. One thing the fix assumed is corrected on the way — `packageManager` did not become inert. The yarn on a developer's PATH is often corepack's own shim, which resolves from `packageManager` which yarn to run, and that yarn then honours `yarnPath`; the documents therefore keep the field and say what reads it. Both candidates being 4.17.1, `yarn --version` cannot tell them apart, so the claim rests on `yarnPath` being honoured by yarn 1 — which the repaired deployments prove on Vercel — and by yarn berry. The doubt stands as stated, unmeasured on a developer machine.
...
Due since the corepack fix of 2026-08-01, delivered in commit `e4deb9a`. The three first entries did not merely read stale: they instructed the reader to run the command that breaks the deployments. `apps/robusta-build/README.md` carries the account and stays the reference.

- changed the Install / clean paragraph of `CLAUDE.md` — it declared yarn 4.17.1 "activated by corepack" and sent the reader to run `corepack enable`; it now names the committed binary, `yarnPath`, the delegation of a yarn on the PATH, what `packageManager` is still read for, why corepack must not be enabled on a build agent, and where the account lives
- changed the Toolchain line of the Dependencies section of `root.archi.md` — `engines.node` corrected from `">=22 <23"` to the manifest's `"22.x"`, `yarnPath` added to the `.yarnrc.yml` settings it enumerates, and its closing sentence rewritten off corepack
- changed the Getting started of `README.md` — the same correction in the terms of a newcomer, in the first commands they run
- changed the green-set sequence of `README.md` — it now names `yarn build:robusta-build`, in the green set since 2026-07-31
