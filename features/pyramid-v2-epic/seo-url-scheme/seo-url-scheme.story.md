...
- A blog roll is paginated by `/p/{n}` at a roll size fixed at 12, a constant of `seopyramids.config.ts` and not a per-site knob; the number never appears in an address, the bare roll address serves page one, and `/p/1` redirects to it permanently, on the blog home as on a category page. At 12 the corpus of 11 articles produces no roll page, so pagination is exercised on the fixture corpus alone until a twelfth article arrives.
...
- 2026-07-31 — The roll size is a constant of 12 and not a per-site knob : "Approve, no roll size : make it fix with a constant parameter of 12". Pourquoi : arbitration of the point raised at the validation of the design doc — at 12 the corpus of 11 articles produces no roll page, and rather than lower the value so pagination becomes visible on today's content, the value is fixed and the shape ships exercised by the fixture corpus alone. The twelfth article then needs no route work.
- 2026-07-31 — `seo-url-scheme.design.md` validated, DRAFT to APPROVED. Pourquoi : the four points raised at the walkthrough were weighed; three stand as designed — the single-article `typescript` roll, the redirects staying dormant while the shell carries `robots: noindex`, and the `next.config.ts` import chain that the first build verifies and that carries a stated fallback. Implementation may start.
