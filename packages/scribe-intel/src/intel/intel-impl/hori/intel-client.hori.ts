...
import { HoriIntent } from './intent.hori.js';

/**
 * Hori is one of the first known spy in History. A military scribe designated
 * as a *maher* (emissary-spy).
 * Papyrus Anastasi I depicts him describing roads, outposts, and enemy contingents.
 */
export class HoriIntelClient implements IntelInterface {
  ...
  createIntentInstance(name: string): IntentInterface {
    ...
    return new HoriIntent(name, this.sender);
  }
...
export function createIntelInstance(collectorUrl: string): IntelInterface {
  ...
  const horiIntelInstance = new HoriIntelClient(sender);
  setMainIntelInstance(horiIntelInstance);
  return horiIntelInstance;
}
