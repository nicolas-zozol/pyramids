...
export class SearchHandler {
    ...
    async performSearch(query: string): Promise<SearchResult> {
        try {
            ...
            const searchInput = await this.page.waitForSelector(config.selectors.searchInput);
            if (!searchInput) {
            ...
            await this.page.waitForSelector(config.selectors.suggestions.container, { timeout: 5000 });
