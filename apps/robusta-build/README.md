...
- Node: 22, declared as `"engines": { "node": "22.x" }` in the root manifest. The form matters and this is the one that works. Vercel honours `engines.node` when it names a major it supports, and falls back to the project's own Node setting when it cannot read the value — a compound range like `">=22 <23"` is not read, so the project ran Node 24 while the manifest said 22 and nobody was told. That is not cosmetic: yarn 4.17.1's CLI bundle does not start on Node 24, failing with `Dynamic require of "util" is not supported` before a single dependency installs. Diagnosed on 2026-08-01 from three consecutive failed deployments.
